...
struct SceneLightData {
    ...
    glm::vec4 pos;  // Position with CTM applied (Not applicable to directional
                    // lights)
    glm::vec4
        dir;  // Direction with CTM applied (Not applicable to point lights)
...
struct SceneMaterial {
    ...
    SceneColor cTransparent;  // Transparency;        used for extra credit
                              // (refraction)
...
struct SceneTransformation {
    ...
    glm::vec3
        translate;    // Only applicable when translating. Defines t_x, t_y, and
                      // t_z, the amounts to translate by, along each axis.
    ...
    float angle;       // Only applicable when rotating.    Defines the angle to
                       // rotate by in RADIANS, following the right-hand rule.
...
struct SceneNode {
    std::vector<SceneTransformation*>
        transformations;  // Note the order of transformations described in lab
