...
#define PARSE_ERROR(e)                              \
    std::cout << ERROR_AT(e) << "could not parse <" \
...
bool ScenefileReader::parseGlobalData(const QJsonObject& globalData) {
    ...
    if (globalData.contains("transparentCoeff")) {
        if (globalData["transparentCoeff"].isDouble()) {
            ...
            std::cout
                << "globalData transparentCoeff must be a floating-point value"
                << std::endl;
            return false;
        }
    }
...
bool ScenefileReader::parseLightData(const QJsonObject& lightData,
    ...
    if (!colorArray[0].isDouble() || !colorArray[1].isDouble() ||
        ...
        std::cout << "light color must contain floating-point values"
                  << std::endl;
        return false;
    }
    ...
    if (lightType == "directional") {
        ...
        if (directionArray.size() != 3) {
            std::cout << "directional light direction must be of size 3"
                      << std::endl;
            return false;
        }
        if (!directionArray[0].isDouble() || !directionArray[1].isDouble() ||
            !directionArray[2].isDouble()) {
            std::cout << "directional light direction must contain "
        ...
        if (!attenuationArray[0].isDouble() ||
            !attenuationArray[1].isDouble() ||
            !attenuationArray[2].isDouble()) {
            std::cout << "ppoint light attenuationCoeff must contain "
                         "floating-point values"
                      << std::endl;
            return false;
        }
        ...
        // parse direction
        if (!lightData["direction"].isArray()) {
            std::cout << "spotlight direction must be of type array"
                      << std::endl;
            return false;
        }
        QJsonArray directionArray = lightData["direction"].toArray();
        ...
        if (!directionArray[0].isDouble() || !directionArray[1].isDouble() ||
            !directionArray[2].isDouble()) {
            std::cout
                << "spotlight direction must contain floating-point values"
                << std::endl;
            return false;
        }
        light->dir.x = directionArray[0].toDouble();
        ...
        QJsonArray attenuationArray = lightData["attenuationCoeff"].toArray();
        if (attenuationArray.size() != 3) {
            std::cout << "spotlight attenuationCoeff must be of size 3"
                      << std::endl;
            return false;
        }
        if (!attenuationArray[0].isDouble() ||
        ...
        if (!lightData["penumbra"].isDouble()) {
            std::cout << "spotlight penumbra must be of type float"
                      << std::endl;
            return false;
        }
...
bool ScenefileReader::parseCameraData(const QJsonObject& cameradata) {
    ...
    if (cameradata["position"].isArray()) {
        ...
        if (position.size() != 3) {
            std::cout << "cameraData position must have 3 elements"
                      << std::endl;
            return false;
        }
    ...
    if (cameradata["up"].isArray()) {
        ...
        if (!up[0].isDouble() || !up[1].isDouble() || !up[2].isDouble()) {
            std::cout << "cameraData up must be a floating-point value"
                      << std::endl;
            return false;
        }
    ...
    if (cameradata.contains("look")) {
        if (cameradata["look"].isArray()) {
            ...
            if (look.size() != 3) {
                std::cout << "cameraData look must have 3 elements"
                          << std::endl;
                return false;
            }
            if (!look[0].isDouble() || !look[1].isDouble() ||
                !look[2].isDouble()) {
        ...
        if (cameradata["focus"].isArray()) {
            ...
            if (focus.size() != 3) {
                std::cout << "cameraData focus must have 3 elements"
                          << std::endl;
                return false;
            }
...
bool ScenefileReader::parseTemplateGroups(const QJsonValue& templateGroups) {
    ...
    for (auto templateGroup : templateGroupsArray) {
        if (!templateGroup.isObject()) {
            std::cout << "templateGroup items must be of type object"
                      << std::endl;
            return false;
        }
...
bool ScenefileReader::parseGroupData(const QJsonObject& object,
                                     SceneNode* node) {
    QStringList optionalFields = {"name",       "translate", "rotate",
                                  "scale",      "matrix",    "lights",
                                  "primitives", "groups"};
    ...
    if (object.contains("matrix")) {
        if (!object["matrix"].isArray()) {
            std::cout << "group matrix must be of type array of array"
                      << std::endl;
            return false;
        }
        ...
        for (auto row : matrixArray) {
            ...
            for (auto val : rowArray) {
                if (!val.isDouble()) {
                    std::cout
                        << "group matrix must contain all floating-point values"
                        << std::endl;
                    return false;
                }
...
bool ScenefileReader::parsePrimitive(const QJsonObject& prim, SceneNode* node) {
    ...
    if (primType == "sphere")
        ...
        std::cout << "unknown primitive type \"" << primType << "\""
                  << std::endl;
        return false;
    }
    ...
    if (prim.contains("ambient")) {
        ...
        if (ambientArray.size() != 3) {
            std::cout << "primitive ambient array must be of size 3"
                      << std::endl;
            return false;
        }
        ...
        for (int i = 0; i < 3; i++) {
            if (!ambientArray[i].isDouble()) {
                std::cout
                    << "primitive ambient must contain floating-point values"
                    << std::endl;
                return false;
            }
    ...
    if (prim.contains("diffuse")) {
        ...
        if (diffuseArray.size() != 3) {
            std::cout << "primitive diffuse array must be of size 3"
                      << std::endl;
            return false;
        }
        ...
        for (int i = 0; i < 3; i++) {
            if (!diffuseArray[i].isDouble()) {
                std::cout
                    << "primitive diffuse must contain floating-point values"
                    << std::endl;
                return false;
            }
    ...
    if (prim.contains("specular")) {
        if (!prim["specular"].isArray()) {
            std::cout << "primitive specular must be of type array"
                      << std::endl;
            return false;
        }
        QJsonArray specularArray = prim["specular"].toArray();
        if (specularArray.size() != 3) {
            std::cout << "primitive specular array must be of size 3"
                      << std::endl;
            return false;
        }
        ...
        for (int i = 0; i < 3; i++) {
            if (!specularArray[i].isDouble()) {
                std::cout
                    << "primitive specular must contain floating-point values"
                    << std::endl;
                return false;
            }
    ...
    if (prim.contains("reflective")) {
        if (!prim["reflective"].isArray()) {
            std::cout << "primitive reflective must be of type array"
                      << std::endl;
            return false;
        }
        QJsonArray reflectiveArray = prim["reflective"].toArray();
        if (reflectiveArray.size() != 3) {
            std::cout << "primitive reflective array must be of size 3"
                      << std::endl;
            return false;
        }
        ...
        for (int i = 0; i < 3; i++) {
            if (!reflectiveArray[i].isDouble()) {
                std::cout
                    << "primitive reflective must contain floating-point values"
                    << std::endl;
                return false;
            }
    ...
    if (prim.contains("transparent")) {
        if (!prim["transparent"].isArray()) {
            std::cout << "primitive transparent must be of type array"
                      << std::endl;
            return false;
        }
        QJsonArray transparentArray = prim["transparent"].toArray();
        if (transparentArray.size() != 3) {
            std::cout << "primitive transparent array must be of size 3"
                      << std::endl;
            return false;
        }
        ...
        for (int i = 0; i < 3; i++) {
            if (!transparentArray[i].isDouble()) {
                std::cout << "primitive transparent must contain "
                             "floating-point values"
                          << std::endl;
                return false;
            }
    ...
    if (prim.contains("shininess")) {
        if (!prim["shininess"].isDouble()) {
            std::cout << "primitive shininess must be of type float"
                      << std::endl;
            return false;
        }
    ...
    if (prim.contains("textureFile")) {
        if (!prim["textureFile"].isString()) {
            std::cout << "primitive textureFile must be of type string"
                      << std::endl;
            return false;
        }
    ...
    if (prim.contains("bumpMapFile")) {
        if (!prim["bumpMapFile"].isString()) {
            std::cout << "primitive bumpMapFile must be of type string"
                      << std::endl;
            return false;
        }
        std::filesystem::path fileRelativePath(
